...
public class Shijimi implements Runnable{
    private BufferedWriter sensorFw = null;
    private BufferedWriter driveFw = null;
    ...
    private void InitFiles(){//ファイルの作成及びFileWriterの取得を行う関数。
        ...
        try {
            ...
            sensorFw = new BufferedWriter(new FileWriter(sensorLogFile,true));
            driveFw = new BufferedWriter(new FileWriter(driveLogFile,true));
        } catch (IOException e) {
    ...
    private void writeLog(String str,boolean useTimeStamp,BufferedWriter fw){
        if(useTimeStamp){
            try {
